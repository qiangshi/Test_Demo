...
public class ScrollLayout extends FrameLayout {
            ...
            new GestureDetector.SimpleOnGestureListener() {
                ...
                public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
                    if (velocityY > FLING_VELOCITY_SLOP) {
                        if (lastFlingStatus.equals(ScrollLayout.Status.OPENED) && -getScrollY() > maxOffset) {
                            lastFlingStatus = ScrollLayout.Status.EXIT;
                            scrollToExit();
                        } else {
                            scrollToOpen();
                            lastFlingStatus = ScrollLayout.Status.OPENED;
                        }
                        return true;
                        ...
                        scrollToClose();
                        lastFlingStatus = ScrollLayout.Status.CLOSED;
                        return true;
                    }
                    return false;
                }
    ...
    private ScrollLayout.Status lastFlingStatus = ScrollLayout.Status.CLOSED;
    ...
    private ScrollLayout.InnerStatus currentInnerStatus = ScrollLayout.InnerStatus.OPENED;
    ...
    public void scrollTo(int x, int y) {
        ...
        if (y == -minOffset) {
            // closed
            if (currentInnerStatus != ScrollLayout.InnerStatus.CLOSED) {
                currentInnerStatus = ScrollLayout.InnerStatus.CLOSED;
                onScrollFinished(ScrollLayout.Status.CLOSED);
            }
        } else if (y == -maxOffset) {
            // opened
            if (currentInnerStatus != ScrollLayout.InnerStatus.OPENED) {
                currentInnerStatus = ScrollLayout.InnerStatus.OPENED;
                onScrollFinished(ScrollLayout.Status.OPENED);
            }
        } else if (isSupportExit && y == -exitOffset) {
            // exited
            if (currentInnerStatus != ScrollLayout.InnerStatus.EXIT) {
                currentInnerStatus = ScrollLayout.InnerStatus.EXIT;
                onScrollFinished(ScrollLayout.Status.EXIT);
            }
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isDraggable && currentInnerStatus == ScrollLayout.InnerStatus.CLOSED) {
            return false;
        }
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (!scroller.isFinished()) {
                    ...
                    currentInnerStatus = ScrollLayout.InnerStatus.MOVING;
            ...
            case MotionEvent.ACTION_CANCEL:
                isAllowPointerIntercepted = true;
                isCurrentPointerIntercepted = false;
                if (currentInnerStatus == ScrollLayout.InnerStatus.MOVING) {
                    return true;
                }
                break;
            case MotionEvent.ACTION_MOVE:
                ...
                } else if (currentInnerStatus == ScrollLayout.InnerStatus.OPENED && !isSupportExit) {
    ...
    public void showOrHide() {
        if (currentInnerStatus == ScrollLayout.InnerStatus.OPENED) {
            scrollToClose();
        } else if (currentInnerStatus == ScrollLayout.InnerStatus.CLOSED) {
            scrollToOpen();
        }
    }
    ...
    public void scrollToOpen() {
        ...
        currentInnerStatus = ScrollLayout.InnerStatus.SCROLLING;
    ...
    public void setOnScrollChangedListener(ScrollLayout.OnScrollChangedListener listener) {
    ...
    public interface OnScrollChangedListener {
         ...
         */
        void onScrollFinished(ScrollLayout.Status currentStatus);
